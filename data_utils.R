...
library(readr)
library(stringr)
suppressWarnings(library(ISOweek))
library(scales)
...
load_data <- function(root_dir) {
  wastewater_data <- read_csv(paste0(root_dir, "/data/WWTP_NewTargets_withPMMoV.csv")) %>%
  ...
  cases_data <- read_csv(paste0(root_dir, "/data/cases-wwtp_2022-11-04.csv")) %>%
    rename(CaseCount = Case) %>%
    process_date() %>%
    select(Code, SampleDate, CaseCount, PopulationSize)
  
  flow_data <- read_csv(paste0(root_dir, "/data/Flow_2022-10-31_edited.csv")) %>%
    process_date() %>%
    select(Code, SampleDate, Flow) %>%
    group_by(Code, SampleDate) %>%
    summarize(Flow = mean(Flow, na.rm = TRUE))
  
  vax_data <- read_csv(paste0(root_dir, "/data/vaxbyweek_111022.csv")) %>%
    rename(OneVaxCount = PPL_One, FullVaxCount = PPL_Full) %>%
    process_date() %>%
    select(Code, SampleDate, OneVaxCount, FullVaxCount)

  wwtp_code_to_name_mapping <- read_csv(paste0(root_dir, "/data/sewershed_cty.csv")) %>%
